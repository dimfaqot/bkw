    ...
    public function updateHutang($id)
    {
        ...
        foreach ($items as $item) {
            ...
            if (!empty($produk->iot_id)) {
                ...
                if ($iotAlokasiCek && in_array($iotAlokasiCek['status_penggunaan'], ['dipakai', 'selesai_menunggu_pembayaran']) && (int)($iotAlokasiCek['transaksi_aktif_id'] ?? 0) !== (int)$id) {
                    ...
                    $db->transRollback();
                    return $this->respond(['status' => 'gagal', 'pesan' => "⛔ Meja/Sewa '{$produk->nama_produk}' sedang aktif digunakan oleh transaksi lain (Invoice: {$invLain}). Meja tidak dapat ditambahkan."], 400);
                }
            }
